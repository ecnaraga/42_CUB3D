...
/*   Updated: 2024/05/15 17:57:02 by galambey         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_fill_textures(t_cub *cub, char *line)
{
	...
	if (id < F)
	{
		...
		ft_cpy_color(cub, line + j, &cub->map.t.color[0]);
	else
		ft_cpy_color(cub, line + j, &cub->map.t.color[1]);
}
...
static void	ft_init_texture(t_cub *cub, char *av)
{
	...
	cub->map.t.tex = mcgic(NULL, sizeof(char *) * 5, MLC);
	if (!cub->map.t.tex)
		ft_exit(cub, "Dynamic allocation failed");
	i = 0;
	while (i < 5)
