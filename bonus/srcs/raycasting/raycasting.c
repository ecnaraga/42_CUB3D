...
/*   Updated: 2024/05/17 15:25:18 by galambey         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_init_sprites_tab(t_cub *cub, t_ray *ray)
{
	int	i;

	i = -1;
	cub->ray.last_sprite = -1;
	while (++i < cub->ray.nb_sprite)
	{
		ray->sprite[i].sprite = 0;
		ray->sprite[i].dist = 0.0;
	}
}

void	ft_draw_sight_bis(t_cub *cub, double hm, int x)
{
	ft_init_sprites_tab(cub, &cub->ray);
	ft_calcul_ray_hwall(cub, ft_deg_to_rad(cub->ray.a));
	ft_calcul_ray_vwall(cub, ft_deg_to_rad(cub->ray.a));
	ft_set_dist(&cub->ray);
	ft_set_sprite(&cub->ray, ft_deg_to_rad(cub->ray.a));
...
void	ft_draw_sight(t_cub *cub, double hm)
{
	...
	while (++x < WIDTH)
	{
		ft_draw_sight_bis(cub, hm, x);
		if (cub->ray.last_sprite > -1)
			ft_draw_sprite(cub, x, hm);
