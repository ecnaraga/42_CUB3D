...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/05/10 17:23:42 by athiebau          #+#    #+#             */
/*   Updated: 2024/05/17 02:14:22 by alix             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static bool	check_borders_bis(t_cub *cub, int i, int j)
{
	...
	if (i > 0 && !ft_strchr(ALLOWD_BRD_B, cub->map.map_final[i - 1][j]))
		return (FALSE);
	if (j > 0 && !ft_strchr(ALLOWD_BRD_B, cub->map.map_final[i][j - 1]))
		return (FALSE);
	if (i < (cub->parsing.len_y - 1) && !ft_strchr(ALLOWD_BRD_B,
			cub->map.map_final[i + 1][j]))
		return (FALSE);
	if (j < (cub->parsing.len_x - 1) && !ft_strchr(ALLOWD_BRD_B,
	...
	while (cub->map.map_final[++i])
	{
		while (cub->map.map_final[i][++j])
		{
			if (cub->map.map_final[i][j] == '0'
				...
				|| cub->map.map_final[i][j] == 'E'
				|| cub->map.map_final[i][j] == 'W'
				|| cub->map.map_final[i][j] == '2'
				|| cub->map.map_final[i][j] == '3')
